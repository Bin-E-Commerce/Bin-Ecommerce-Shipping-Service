...
import type { SellerPickupAddressResponse } from "../clients/seller-shop.client";
import type {
...
import { GHN_PROVIDER, SHIPPING_PROVIDER } from "../types/shipping.types";
import type {
...
export class ShippingService {
  ...
  async createForSeller(
    ...
    const pickup = await this.sellerShopClient.getDefaultPickupAddress(shopId);
    const pickupAddress = this.toShippingAddress(pickup);
  ...
  async getForSeller(
    ...
    const shopId = await this.sellerShopClient.getOwnedShopId(currentUser);
    const shipment = await this.repository.findByOrderAndShop(orderId, shopId);
    if (!shipment)
  ...
  private async advanceDemoShipment(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!currentPoint)
        throw new ConflictException("Shipment chưa có dữ liệu lộ trình demo.");
  ...
  async handleWebhook(body: Record<string, unknown>): Promise<void> {
    ...
    if (!providerTrackingId || !statusText) {
      this.logger.warn("GHN webhook hợp lệ nhưng thiếu OrderCode/Status.");
      return;
    }
    const shipment = await this.repository.getEntityRepository().findOne({
      where: [
    ...
    await this.applyProviderUpdate(
      shipment.id,
      {
        trackingId: this.text(body.label_id) ?? shipment.providerTrackingId,
        providerOrderReference: partnerReference,
        providerStatusCode: null,
  ...
  async syncActiveShipments(): Promise<void> {
    ...
    await Promise.allSettled(
      candidates.map((shipment) => this.syncShipment(shipment.id, "POLL")),
    );
  }
  ...
  private async applyProviderUpdate(
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      shipment.providerOrderReference =
        update.providerOrderReference ?? shipment.providerOrderReference;
      ...
      if (update.currentLocation) {
        shipment.currentLatitude = String(update.currentLocation.latitude);
        shipment.currentLongitude = String(update.currentLocation.longitude);
        shipment.currentLocationLabel = update.currentLocation.label;
      }
      if (update.routePoints?.length) shipment.routePoints = update.routePoints;
      ...
      const event = eventRepository.create({
        ...
        fromStatus: previousStatus === nextStatus ? null : previousStatus,
        toStatus: nextStatus,
  ...
  private ensureSellerContext(
    ...
    if (!currentUser.userId || !currentUser.email)
      throw new UnauthorizedException("Bạn cần đăng nhập để quản lý vận đơn.");
    const canRead =
  ...
  private normalizeRoutePoints(
    ...
    return points.every((point) => point !== null)
      ? points.filter(
          (
            point,
          ): point is { latitude: number; longitude: number; label: string } =>
            point !== null,
        )
      : null;
  }
  ...
  private providerOrderReference(orderNumber: string, shopId: string): string {
