...
export class GhnMasterDataClient {
    ...
    constructor(config: ConfigService) {
        ...
        if (this.baseUrl !== 'https://dev-online-gateway.ghn.vn')
            throw new Error(
                'GHN_BASE_URL phải trỏ tới GHN Test trong phase này.',
            );
    ...
    async listProvinces(): Promise<GhnProvinceOption[]> {
        ...
        return rows
            .map((row) => ({
                id: this.number(row.ProvinceID),
                name: this.string(row.ProvinceName),
            }))
            .filter(
                (item): item is GhnProvinceOption =>
                    item.id !== null && item.id > 0 && item.name !== null,
            );
    }
    ...
    async listDistricts(provinceId: number): Promise<GhnDistrictOption[]> {
        ...
        return rows
            .map((row) => ({
                id: this.number(row.DistrictID),
                name: this.string(row.DistrictName),
                provinceId,
            }))
            .filter(
                (item): item is GhnDistrictOption =>
                    item.id !== null && item.id > 0 && item.name !== null,
            );
    }
    ...
    async listWards(districtId: number): Promise<GhnWardOption[]> {
        ...
        return rows
            .map((row) => ({
                code: this.string(row.WardCode),
                name: this.string(row.WardName),
                districtId,
            }))
            .filter(
                (item): item is GhnWardOption =>
                    item.code !== null && item.name !== null,
            );
    }
    ...
    private assertId(value: number, label: string): void {
        if (!Number.isInteger(value) || value < 1)
            throw new BadRequestException(`Mã ${label} GHN không hợp lệ.`);
    }
    ...
    private async getRows(
        ...
        if (cached && cached.expiresAt > Date.now())
            return cached.value as UnknownRecord[];
        if (cached) this.cache.delete(cacheKey);
        const running = this.inFlight.get(cacheKey);
        if (running) return running as Promise<UnknownRecord[]>;

        const request = this.request<UnknownRecord>(
            path,
            ...
            .then((payload) => {
                const rows = Array.isArray(payload.data)
                    ? payload.data.map((item) => this.asRecord(item))
                    : [];
    ...
    private async request<T extends UnknownRecord>(
        path: string,
        method: 'GET' | 'POST',
        body?: unknown,
    ): Promise<T> {
        ...
        if (!response.ok || (code !== null && code !== 200))
            throw new BadGatewayException(
                this.string(payload.message) ??
                    'GHN từ chối request master data.',
            );
        return payload;
    }
