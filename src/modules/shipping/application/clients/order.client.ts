...
export class OrderClient {
    ...
    async getSellerShippingContext(
        orderId: string,
        userId: string,
        shopId: string,
    ...
    async getReturnShippingContext(
        returnId: string,
    ): Promise<ReturnShippingOrderContext> {
    ...
    async updateReturnShippingCost(
        returnId: string,
        amount: string,
    ): Promise<void> {
        await this.request<{ id: string }>(
    ...
    private async request<T>(
        ...
        try {
            ...
            throw new BadGatewayException(
                'Order Service chưa sẵn sàng. Vui lòng thử lại sau.',
            );
        }

        if (response.status === 404)
            throw new NotFoundException(
                'Không tìm thấy đơn hàng trong phạm vi tài khoản.',
            );
        if (!response.ok)
            throw new BadGatewayException(
                'Không thể lấy dữ liệu đơn hàng lúc này.',
            );
