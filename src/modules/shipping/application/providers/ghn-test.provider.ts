...
import { GHN_PROVIDER } from '@/modules/shipping/application/types/shipping.types';
...
export class GhnTestProvider implements ShippingProvider {
    ...
    async createShipment(input: CreateShipmentInput): Promise<CreatedShipment> {
        ...
        if (!trackingId)
            throw new BadGatewayException(
                'GHN trả về response thiếu mã vận đơn.',
            );
    ...
    private async validateSelection(
        ...
        const province = provinces.find(
            (item) => item.id === selection.provinceId,
        );
        const namedProvince = provinces.find(
            (item) =>
                this.normalizeName(item.name) ===
                this.normalizeName(provinceName),
        );
        if (
            !province ||
            !namedProvince ||
        ...
        const district = districts.find(
            (item) => item.id === selection.districtId,
        );
        if (!district)
            throw new BadRequestException(
                'Quận/huyện GHN đã chọn không thuộc tỉnh/thành phố.',
            );
        const wards = await this.masterDataClient.listWards(
    ...
    private normalizeName(value: string): string {
        return value
            ...
            .toLowerCase()
            .replace(
                /thanh pho|tinh|quan|huyen|thi xa|phuong|xa|thi tran/g,
                ' ',
            )
            .replace(/[^a-z0-9]+/g, ' ')
            .trim()
    ...
    private packageData(input: CreateShipmentInput): {
        ...
        const length = Math.max(
            ...input.items.map((item) => item.packageLengthCm),
        );
        const width = Math.max(
            ...input.items.map((item) => item.packageWidthCm),
        );
    ...
    private createPartnerReference(
        orderNumber: string,
        shopId: string,
