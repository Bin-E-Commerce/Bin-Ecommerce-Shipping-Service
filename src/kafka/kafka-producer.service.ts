...
export class KafkaProducerService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(config: ConfigService) {
    const kafka = new Kafka({
      ...
      brokers: config.get<string>("KAFKA_BROKERS", "localhost:29092").split(",").map((broker) => broker.trim()),
      retry: { retries: 3 },
    });
  ...
  async publish<TPayload>(topic: string, key: string, payload: TPayload): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to publish Kafka message topic=${topic} key=${key}: ${String(error)}`);
    }
  }
}
